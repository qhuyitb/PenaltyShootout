...
public class Client {
    ...
    private void handleMessage(Message message) {
        ...
        switch (message.getType()) {
            ...
            case "register_success":
                Platform.runLater(() -> {
                    if (registerController != null) {
                        String successMessage = (String) message.getContent();
                        registerController.showSuccess(successMessage != null ? 
            ...
            case "register_failure":
                Platform.runLater(() -> {
                    if (registerController != null) {
                        ...
                        registerController.showError(errorMessage != null ? 
            ...
            case "match_start":
                ...
                break;
            case "animate_shoot_vao":
                Platform.runLater(() -> {
                    if (gameRoomController != null) {
                        String animationData = (String) message.getContent();
                        String[] parts = animationData.split("\\|");
                        if (parts.length >= 2) {
                            String shootDirection = parts[0];
                            String keeperDirection = parts[1];
                            gameRoomController.animateShootVao(shootDirection, keeperDirection);
                        }
                    }
                });
                break;

            case "animate_shoot_khong_vao":
                Platform.runLater(() -> {
                    if (gameRoomController != null) {
                        String animationData = (String) message.getContent();
                        String[] parts = animationData.split("\\|");
                        if (parts.length >= 2) {
                            String shootDirection = parts[0];
                            String keeperDirection = parts[1];
                            gameRoomController.animateShootKhongVao(shootDirection, keeperDirection);
                        }
                    }
                });
                break;
    ...
    // Getter cho RegisterController
    public RegisterController getRegisterController() {
        return registerController;
    }
    
    // Setter cho RegisterController
